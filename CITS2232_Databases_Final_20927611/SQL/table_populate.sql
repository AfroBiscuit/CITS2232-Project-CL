### REPLACE ALL INSTANCES OF %%data_path%% with the path to the folder containing offices.csv, streets.csv and postcodes.csv  ####
...
BEGIN
	...
	`staffloop`: LOOP
		...
		SET @userpassword = CONCAT(10000000 + RAND()*89999999 DIV 1);
		...
		SET @phone = 400000000 + RAND()*99999999 DIV 1;
		INSERT INTO `staff` (password, isAdmin, dateRegistered, firstName, lastName, email, streetAddress, suburb, postcode, state, phone)
		VALUES(@userpassword,0,NOW(),rand_firstname(), rand_lastname(),CONCAT(@staffid,'@staff.centrelink.gov.au'),CONCAT(((RAND()*130) DIV 1),' ',@street),@suburb,@postcode,@state,CONCAT('0',@phone));
...
DELIMITER ;
	...
	SET @phone = 400000000 + RAND()*99999999 DIV 1;
	 INSERT INTO `staff` (password, isAdmin, dateRegistered, firstName, lastName, email, streetAddress, suburb, postcode, state, phone)
	VALUES('admin',1,NOW(),rand_firstname(), rand_lastname(),CONCAT(@staffid,'@staff.centrelink.gov.au'),CONCAT(((RAND()*130) DIV 1),' ',@street),@suburb,@postcode,@state,CONCAT('0',@phone));
...
CALL make_rand_memberships(150);
CALL make_rand_memberships(100);
CALL make_rand_memberships(50);